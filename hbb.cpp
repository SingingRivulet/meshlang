...
HBB::AABB * HBB::add(const HBB::vec & from,const HBB::vec & to,void * data){
    auto p=createAABB();
    p->from=from;
    p->to=to;
    p->data=data;
...
void HBB::AABB::fetchByStep(
    float step,
    void(*callback)(AABB *,void *),
    void * arg
){
    if(left && left->onStep(step)){
        if(left->isDataNode())
            callback(left,arg);
        else
            left->fetchByStep(step,callback,arg);
    }
    if(right && right->onStep(step)){
        if(right->isDataNode())
            callback(right,arg);
        else
            right->fetchByStep(step,callback,arg);
    }
}
HBB::HBB(){
    poolInit();
